...
var good = {
    ...
    addgood: function() {
		...
		$.ajax({
            ...
            cache: false,
			async:false,
            url: this.Typesurl + this.goodid + "&trcount="+this.trcount+"&weight="+ this.weight.substring(0,this.weight.length-1),
            data: "",
            success: function(json) {
		        ...
		        for (var i = 0; i < json.attrs.length; i++) {
		            ...
		            if(json.attrs[i][1].substring(0,json.attrs[i][1].length-1)==json.weight){str+=' selected=selected';}
